...
<template>
    <li :class="{ 'hidden': props.currentPage !== props.pageNumber && !props.text && !props.sticky }" class="h-full md:block">
        <button class="relative inline-flex items-center h-full px-3 group hover:text-neutral-900 hover:dark:text-neutral-50 disabled:dark:text-neutral-500 disabled:text-neutral-300" :class="{ 'text-neutral-900 dark:text-neutral-50 bg-gray-50 dark:bg-neutral-900': props.currentPage === props.pageNumber, }" 
            :disabled="props.disabled ?? false">
            <span>
                {{ props?.text ?? props.pageNumber }}
            </span>

            <span v-if="!props.text || props.underline"
                ...
                :class="{'left-0 w-full border-l border-r' : props.currentPage === props.pageNumber, 'left-1/2' : props.currentPage !== props.pageNumber}">
            </span>
        </button>
    </li>
</template>