...
onMounted(() => {
    ...
    if (!alreadyExists) {
        ...
        newHeightArr = props.heights.map((height) => (height.toastId === props.toast.id ? { ...height, height: newHeight } : height));
    }
...
watchEffect((onInvalidate) => {
    if ((props.toast.promise && toastType.value === 'loading') || props.toast.duration === Infinity || props.toast.type === 'loading') {
        return;
    }
...
onMounted(() => {
    ...
    if (toastRef.value) {
        ...
        const newHeights = [{ toastId: props.toast.id, height, position: props.toast.position! }, ...props.heights];
...
<template>
    ...
    >
        ...
        <template v-if="toast.component">
            <component :is="toast.component" v-bind="toast.componentProps" :onCloseToast="handleCloseToast" />
        </template>

        <template v-else>
            <template v-if="toastType !== 'default' || toast.icon || toast.promise">
                <div data-icon="" :class="cn(classes?.icon, toast?.classes?.icon)">
                    <template v-if="(toast.promise || toastType === 'loading') && !toast.icon">
            ...
            <div data-content="" :class="cn(classes?.content, toast?.classes?.content)">
                ...
                <template v-if="toast.description">
                    <div
                        data-description=""
                        :class="cn(descriptionClass, toastDescriptionClass, classes?.description, toast.classes?.description)"
                    >
