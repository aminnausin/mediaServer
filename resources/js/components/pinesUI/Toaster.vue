...
import { computed, nextTick, ref, toRaw, useAttrs, watch, watchEffect } from 'vue';
...
const filteredToasts = computed(() => {
    return (pos: string, index: number) => toasts.value.filter((toast) => (!toast.position && index === 0) || toast.position === pos);
});
