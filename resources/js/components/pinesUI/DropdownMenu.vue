...
<template>
    <OnClickOutside @trigger="$emit('toggleDropdown', false)">
        <Transition
            ...
            <div v-if="props.dropdownOpen" class="absolute top-0 z-50 w-56 mt-12 -translate-x-1/2 left-1/2" v-cloak>
                <div v-if="userData" class="p-1 mt-1 bg-white dark:bg-neutral-800 border rounded-md shadow-md border-neutral-200/70 dark:border-neutral-700 text-neutral-700 dark:text-neutral-100">
                    <div class="px-2 py-1.5 text-sm font-semibold">My Account</div>
                    <div class="h-px my-1 -mx-1 bg-neutral-200"></div>
                    <section v-for="(group, groupIndex) in dropDownItemsAuth" :key="groupIndex">
                        <div v-if="groupIndex !== 0 && groupIndex !== dropDownItemsAuth.length" class="h-px my-1 -mx-1 bg-neutral-200"></div>
                        <DropdownItem v-for="(item, index) in dropDownItemsAuth[groupIndex]" :key="index" :linkData="item" :selected="$route.name === item.name" :external="item?.external">
                            <template #icon>
                                <component :is="item.icon" width="24" height="24" viewBox="0 0 24 24"
                ...
                <div v-else class="p-1 mt-1 bg-white dark:bg-neutral-800 border rounded-md shadow-md border-neutral-200/70 dark:border-neutral-700 text-neutral-700 dark:text-neutral-100">
                    <DropdownItem v-for="(item, index) in dropDownItems" :key="index" :linkData="item" :selected="$route.name === item.name" :external="item?.external">
                        <template #icon>
                            <component :is="item.icon" width="24" height="24" viewBox="0 0 24 24"
                            fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                            stroke-linejoin="round" class="w-4 h-4 mr-2"/>
