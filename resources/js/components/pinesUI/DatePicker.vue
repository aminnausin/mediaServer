...
<template>
    <OnClickOutside class="relative z-30" @trigger="datePicker.toggleDatePicker(false)">
        <input ref="datePickerInput" 
            :class="'h-10 px-3 py-2 rounded-md shadow-sm block mt-1 w-full ' +
        ...
        <Transition
            ...
            >
            <div v-if="datePicker.datePickerOpen" :class="'absolute left-0 rounded-md shadow-sm ' +
                ...
                <div class="flex items-center justify-between mb-2">
                    ...
                    <div>
                        <button @click="datePicker.datePickerPreviousMonth()" type="button"
                            ...
                            <svg class="inline-flex w-6 h-6 text-gray-400 dark:text-neutral-200" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M15 19l-7-7 7-7" />
                            </svg>
                        </button>
                        <button @click="datePicker.datePickerNextMonth()" type="button"
                            class="inline-flex p-1 transition duration-100 ease-in-out rounded-full cursor-pointer focus:outline-none focus:shadow-outline hover:bg-gray-100 dark:hover:bg-neutral-900">
                            <svg class="inline-flex w-6 h-6 text-gray-400 dark:text-neutral-200" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="grid grid-cols-7">
                    <div v-for="(blankDay, index) in datePicker.datePickerBlankDaysInMonth" :key="index"
                        class="p-1 text-sm text-center border border-transparent"></div>
                    <div v-for="(day, dayIndex) in datePicker.datePickerDaysInMonth" :key="dayIndex"
                        class="px-0.5 mb-1 aspect-square">
                        <div @click="datePicker.datePickerDayClicked(day)"
                            :class="{
                                'bg-neutral-200 dark:bg-neutral-800/70 dark:hover:bg-neutral-900': datePicker.datePickerIsToday(day) == true,
                                'text-gray-600 dark:text-neutral-400 hover:bg-neutral-200 dark:hover:text-neutral-100 dark:hover:bg-neutral-900': datePicker.datePickerIsToday(day) == false && datePicker.datePickerIsSelectedDate(day) == false,
