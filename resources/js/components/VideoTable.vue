...
<template>
    <table class="w-full overflow-x-hidden flex flex-col gap-2">
        ...
        <tbody class="flex w-full flex-wrap gap-2 [&>*:nth-child(odd)]:bg-violet-200 hover:[&>*:nth-child(odd)]:bg-violet-300 dark:[&>*:nth-child(odd)]:bg-zinc-700 dark:hover:[&>*:nth-child(odd)]:bg-indigo-900">
            <VideoCard v-for="(video, index) in stateFilteredPlaylist" :key="index" :video="video" :index="index" :currentID="stateVideo.id" @playByID="playlistFind"/>
        </tbody>
        <hr>
        <section>
            {{ `Showing ${stateFilteredPlaylist.length} of ${stateFolder.videos.length} Result${stateFolder.videos.length > 1 ? 's' : ''} ${searchQuery ? '(Filtered)' : ''}` }}
        </section>
    </table>
</template>