...
<template>
    <tr :class="{ 'ring-violet-600/70 ring-[0.125rem]': (props.currentID === props.video.id) }"
        ...
        <td class="flex gap-12 w-full">
            <p class="vid-row-title w-full xl:min-w-48 sm:max-w-1/2 sm:w-auto flex justify-between items-center gap-12 sm:gap-0"><span class="sm:hidden">Title: </span><span class="line-clamp-1">{{ props.video.attributes.title ?? props.video.attributes.name }}</span></p>
            <p class="vid-row-duration truncate hidden sm:block">{{ props.video.attributes.duration ?? 'N/A' }}</p>
        </td>
        <td class="flex gap-12 w-full sm:justify-end">
            <p class="vid-row-title truncate hidden sm:block">{{ props.video.attributes.view_count ?? 'No Views' }}</p>
