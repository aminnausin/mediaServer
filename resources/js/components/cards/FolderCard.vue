...
<template>
    <RouterLink :to="folderLink"
        ...
        <section class="flex justify-between gap-4 w-full">
            <h2 class="text-xl w-full truncate" :title="props.folder.attributes.name">{{ props.folder.attributes.name }}</h2>
        ...
        <section class="flex flex-col sm:flex-row sm:justify-between w-full">
            <h3 class="text-neutral-500 w-full text-wrap truncate sm:text-nowrap" :title="`${props.folder.attributes.file_count} Episode${props.folder.attributes.file_count !== 1 ? 's' : ''}`">
