...
<template>
    ...
    <section v-if="selectedSideBar === 'folders'" id="list-content-folders" class="flex space-y-2 flex-wrap">
        <FolderCard v-for="folder in folders" :key="folder.id" :folder="folder" :categoryName="stateDirectory.name" :stateFolderName="stateFolder?.name" @clickAction="handleShare"/>
    </section>
