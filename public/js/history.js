...
function parseHistory(data, empty = true) {
    ...
    let recordTemplate = function(videoName, folderName, timeSpan, date) {
        return `
            <div class="flex flex-wrap rounded-xl dark:bg-neutral-800 dark:hover:bg-neutral-700 hover:bg-slate-200 bg-slate-100  dark:text-white shadow p-3 w-full divide-y divide-gray-300 group flex-grow cursor-pointer">
    ...
    for (let recordCount = 0; recordCount < data.length; recordCount++) {
        ...
        let recordElement = recordTemplate(videoName, folderName, timeSpan, `${rawDate.toLocaleDateString([], {year: "numeric", month: '2-digit', day: '2-digit', hour: '2-digit', hour12:false, minute:'2-digit'})}`);
        ...
        $(destination).children().last().on('click', () => {
